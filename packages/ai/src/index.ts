...
export class CloudflareProvider implements StructuredModelProvider {
  ...
  constructor(private readonly ai: AiBinding, private readonly model = "@cf/qwen/qwen3-30b-a3b-fp8") {}
...
export class MistralProvider implements StructuredModelProvider {
  ...
  constructor(private readonly apiKey: string, private readonly model = "mistral-small-2603") {}
...
export class OpenAIProvider implements StructuredModelProvider {
  ...
  async generate<T>(request: StructuredGenerationRequest<T>): Promise<ModelResult<T>> {
    ...
    if (!response.ok) {
      ...
      throw new Error(`OpenAI request failed with ${response.status}${requestId ? ` (${requestId})` : ""}.`);
    }
    const body = await response.json() as {
      id?: string;
      output_text?: string;
