...
const profile: BrandProfile = {
  ...
  brandName: "FocusKit",
  summary: "A visual productivity planning system that helps remote workers and creative teams organize priorities, reduce distraction, and plan focused work every week.",
  ...
  evidence: [
    { id: "e1", sourceUrl: "https://example.com", excerpt: "FocusKit Planner helps remote workers and creative teams plan focused work every week.", kind: "website" },
    { id: "e2", sourceUrl: "https://example.com/how-it-works", excerpt: "A visual weekly tutorial with affiliate link tracking, creator samples in stock, approved claims, and disclosure guidelines.", kind: "website" },
  ],
};
...
describe("deterministic scoring", () => {
  ...
  it("abstains for SparseBrand without inventing a North Star or score", () => {
    const sparse: BrandProfile = {
      ...
      targetCustomers: [],
      customerNeeds: [],
      evidence: [{ id: "s1", sourceUrl: "https://sparse.example", excerpt: "Welcome to SparseBrand.", kind: "website" }],
    };
  ...
  it("abstains for an unfocused broad multi-category brand", () => {
    const broad: BrandProfile = {
      ...
      summary: "A multi-category retailer offering a wide range of unrelated products for everyone across home, beauty, electronics, and food.",
      products: [
        { name: "Lamp", category: "home" }, { name: "Serum", category: "beauty" },
        { name: "Headphones", category: "electronics" }, { name: "Snacks", category: "food" },
      ],
  ...
  it.each([
    ...
    ["service", { products: [{ name: "Strategy Sprint", category: "service" }], purchaseFriction: "high" as const }],
    ["e-commerce", { products: [{ name: "FocusKit Planner", category: "productivity", priceText: "$49" }], purchaseFriction: "low" as const }],
  ])("keeps readiness evidence-specific for %s brands", (_name, overrides) => {
    const dimensions = scoreReadiness({ ...profile, ...overrides });
    expect(dimensions.find((item) => item.key === "tracking-readiness")?.evidenceIds).toContain("e2");
    expect(dimensions.find((item) => item.key === "sample-fulfillment")?.evidenceIds).toContain("e2");
  ...
  it("caps overall readiness when critical operational evidence is unknown", () => {
    const unproven = { ...profile, evidence: profile.evidence.map((item) => ({ ...item, excerpt: item.excerpt.replace(/affiliate link tracking, creator samples in stock, approved claims, and disclosure guidelines/i, "a clear weekly planning method") })) };
