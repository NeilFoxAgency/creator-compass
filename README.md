...
1. Validates and safely reads up to five useful public pages.
2. Builds a structured brand profile with attributable evidence and visible unknowns.
3. Scores ten readiness dimensions and creator territories in deterministic TypeScript.
4. Retrieves a bounded candidate set from a reviewed 70-territory taxonomy.
5. Uses Cloudflare Workers AI for high-volume extraction and candidate work, with Mistral as an optional fallback.
6. Uses `gpt-5.6-luna` through the OpenAI Responses API as the final strategic adjudicator.
7. Returns exactly 3 Core, 2 Adjacent, 1 Experimental, and 2 Risk directions.
8. Persists shareable reports in D1 and KV, with queued processing and honest fallback behavior.
9. Lets a blocked-site user continue from pasted text instead of receiving a fabricated report.
10. Offers optional, user-triggered YouTube example discovery without making the core report depend on YouTube.
