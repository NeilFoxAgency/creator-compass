...
const cases: Array<{ profile: BrandProfile; expectedAny: string[]; expectedReadiness?: string }> = [
  ...
  { profile: base("ManyThings", "multi-category retailer", "A broad online retailer selling home, travel, fitness, and pet products.", { targetCustomers: [], customerNeeds: [], unknowns: ["primary audience", "hero offer", "tracking readiness"] }), expectedAny: ["home-organization", "travel-planning", "fitness-education", "pet-care"] },
...
const results = cases.map(({ profile, expectedAny, expectedReadiness }, index) => {
  ...
  const passed = parsed.success && unique && evidenceCoverage && !prohibitedClaims && plausibleCore && readinessExpected && classCounts.core === 3 && classCounts.adjacent === 2 && classCounts.experimental === 1 && classCounts.risk === 2;
  return { case: profile.brandName, passed, schemaValid: parsed.success, uniqueTerritories: unique, evidenceCoverage, prohibitedClaims, plausibleCore, readinessExpected, classCounts, northStar: report.northStar.territoryId, readinessStatus: report.readinessSummary.status };
});

await mkdir("outputs/evaluation", { recursive: true });
await writeFile("outputs/evaluation/evaluation.json", `${JSON.stringify({ generatedAt: new Date().toISOString(), methodology: "2026.07.1", passed: results.every((item) => item.passed), cases: results }, null, 2)}\n`);
